...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        ...
        for (String name:customersOrders.keySet()) {
            ...
            for (double order:customersOrders.get(name)) customerSum+=order;
            if (customerSum>maxOrder){
                maxOrder=customerSum;
                customerName=name;
            }
        }
    ...
    void removeUnprofitableOrders() {
        ...
        for (String name:customersOrders.keySet()){
            double orderSum=0;
            for (double order:customersOrders.get(name)) orderSum+=order;
            if (orderSum<5000) names.add(name);
        }
